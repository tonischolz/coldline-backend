...
import { createDraftInvoice } from "./invoiceNinja.js";
...
/** Creates a draft invoice for a just-booked order and attaches the result
 *  to the order record. Never throws - invoicing hiccups shouldn't undo
 *  a real booking, so failures are logged and noted on the order instead
 *  of blocking the customer's confirmation. */
async function attachInvoice(clientConfig, order) {
  try {
    const { invoiceId, invoiceNumber, needsPricing } = await createDraftInvoice(clientConfig, order);
    order.invoiceId = invoiceId;
    order.invoiceNumber = invoiceNumber;
    order.invoiceNeedsPricing = needsPricing;
  } catch (err) {
    console.error("Invoice Ninja invoice creation failed:", err);
    order.invoiceError = "Invoice wasn't created automatically - add it manually in Invoice Ninja.";
  }
}
...
export async function runTool(clientConfig, toolName, toolInput) {
  ...
  switch (toolName) {
    ...
    case "create_order": {
      const order = createOrder({ clientId, ...toolInput }, clientConfig);
      await attachInvoice(clientConfig, order);
      return order;
    }

    case "create_recurring_order": {
      const order = createRecurringOrder({ clientId, ...toolInput }, clientConfig);
      await attachInvoice(clientConfig, order);
      return order;
    }
